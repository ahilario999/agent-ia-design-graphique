...
export default function MediasSociaux() {
  ...
  return (
    <div className="medias glass stagger-7">
      ...
      {/* Image Instagram — pleine hauteur, cliquable */}
      <a
        href={post?.permalink || INSTAGRAM_URL}
        target="_blank"
        rel="noopener noreferrer"
        className="medias__insta-post"
        aria-label="Voir le dernier post Instagram"
      >
        {loading && (
          <div className="medias__insta-skeleton" />
        )}

        {!loading && post?.imageUrl && (
          <img
            src={post.imageUrl}
            alt="Dernier post Instagram"
            className="medias__insta-img"
          />
        )}

        {!loading && !post && (
          <div className="medias__insta-placeholder">
            <svg width="32" height="32" viewBox="0 0 24 24" fill="rgba(255,255,255,0.3)">
            ...
            <span>@design_graphique_lacite</span>
          </div>
        )}

        {/* Overlay au hover */}
        <div className="medias__insta-overlay">
